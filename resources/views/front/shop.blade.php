                                        ...
                                        <div class="accordion-item">
                                            ...
                                            @else
                                                <a href="{{ route('front.shop', $category->slug) }}"
                                                    class="nav-item nav-link {{ $categorySelected == $category->id ? 'text text-info' : '' }}">{{ $category->name }}</a>
                                            @endif
                                                        ...
                                                        <div class="navbar-nav">
                                                            @foreach ($category->sub_category as $subCategory)
                                                                <a href="{{ route('front.shop', [$category->slug, $subCategory->slug]) }}"
                                                                    class="nav-item nav-link {{ $SubCategorySelected == $subCategory->id ? 'text text-info' : '' }}">{{ $subCategory->name }}</a>
                                                            @endforeach
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                @foreach ($brands as $brand)
                                    <div class="form-check mb-2">
                                        <input class="form-check-input brand-label" type="checkbox" name="brand[]"
                                            value="{{ $brand->id }}"
                                            {{ in_array($brand->id, $brandArray) ? 'checked' : '' }}
                                            id="brand-{{ $brand->id }}">
                                        ...
                                        <div class="card-body text-center mt-3">
                                            ...
                                            <div class="price mt-2">
                                                ...
                                                @if ($product->compare_price > 0)
                                                    <span
                                                        class="h6 text-underline"><del>${{ $product->compare_price }}</del>
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
